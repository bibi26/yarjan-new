@extends('home')
@section('content')
@section('title', __('titles.resetPass'))
             ...
             box-shadow: 0px 2px 18px -5px rgba(0,0,0,0.75); background-color: white;">
                        <div class="col-lg-12" id="dv_sign">
                            <div class="row">
                                ...
                                <span
                                    style="font-weight: bold;color: white;font-size: 18px;"> تغییر رمز عبور</span>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-lg-12">
                                    @include('partials.alerts')
                                    {!! Form::open(['route'=>'resetPass.store','style'=>'padding: 10px','id'=>'signUpForm']) !!}

                                    <div class="form-group {{ $errors->has('pass1') ? 'has-error' : '' }}">
                                        {!! Form::label(' رمز عبور جدید :','',[]) !!}
                                        {!! Form::password('pass1', ['class'=>'form-control','style'=>'direction: ltr']) !!}
                                        <span class="text-danger small ">{{ $errors->first('pass1') }}</span>
                                    </div>
                                    <div class="form-group  {{ $errors->has('pass2') ? 'has-error' : '' }}">
                                        {!! Form::label('  تکرار رمز عبور جدید :','',[]) !!}
                                        {!! Form::password('pass2' , ['class'=>'form-control','style'=>'direction: ltr']) !!}
                                        <span class="text-danger small ">{{ $errors->first('pass2') }}</span>
                                    </div>
                                        @if (Request::segment(2))
                                            {{ Form::hidden('verifyCode', Request::segment(2)) }}
                                        @endif
